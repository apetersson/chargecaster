# Chargecaster Pi Autoresearch

Goal:
- find forecast feature changes that reduce downstream cost when replayed through the existing Chargecaster simulator
...
Primary metric:
- `cost_delta_eur`
- this is `candidate_projected_cost_eur - hybrid_projected_cost_eur`
- lower is better
...
- `backend/scripts/replay-load-forecast.ts` only if the harness itself needs fixing
- avoid broad runtime changes outside the forecast path unless they are required for deterministic offline evaluation

Avoid:
- redesigning `backend/src/simulation/optimal-schedule.ts` early
- changing the benchmark contract casually
- optimizing only for MAE if replayed cost gets worse

Keep criteria:
- keep a change only if `cost_delta_eur` improves materially
...
- Weighting training samples toward economically risky periods helped: a focused mix of low-solar, top-price, and transition-hour weights beat the plain `0.19` model, while broader / more aggressive weighting schemes lost ground.
...
- Fewer trees kept improving the primary metric on the weighted / regularized model through `300 -> 250 -> 200 -> 150 -> 100 -> 80 -> 70`, but MAE and economic-hour error drift upward steadily.
...
- `50` trees looked too aggressive: it improved the replay metric again, but MAE / economic-hour error jumped enough to look like benchmark-specific under-forecasting, so treat that region as suspect.
...
- A simple baseline-floor / target-distillation attempt on risky hours lost too much replay-cost gain.
- Clearly too aggressive / overfit directions so far: `alpha=0.15`, `alpha=0.18` before future-solar removal, removing all solar features, and collapsing solar magnitude to a daylight flag. Those lowered replay cost by making demand look unrealistically small.
